			       ...
			       ulong count, void *buf)
{
	...
	// printf("SPI NAND Count: %lu/%lu\n", count, pagesize);
	// printf("SPI NAND Sector: %lu\n", sector);
	while (remain) {
		ulong count_in_page = min(remain, pagesize - (sector % pagesize));
		ulong current_page = sector / pagesize;
		// printf("SPI NAND Page: %lu/%lu\n", current_page, count_in_page);
		if (spi0_nand_switch_page(current_page) != 0)
		{
			// printf("Page switch error\n");
			return 0;
		}
			    ...
			    bool allow_raw)
{
    ...
    if (IS_ENABLED(CONFIG_SPL_LOAD_FIT) &&
		...
		if (load->read(load, offset, spl_image->size, (void *)spl_image->load_addr) == 0)
			ret = -EINVAL;
	}
